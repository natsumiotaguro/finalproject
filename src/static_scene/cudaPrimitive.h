...
class CudaPrimitive {
 public:
   ...
   */
  virtual CudaBBox get_bbox() const = 0;
   ...
   * \return true if the given ray intersects with the primitive,
             false otherwise
   */
  virtual bool intersect(const CudaRay& r) const = 0;
   ...
   * \return true if the given ray intersects with the primitive,
             false otherwise
   */
  virtual bool intersect(const CudaRay& r, CudaIntersection* i) const = 0;
   ...
   */
  virtual void draw(const CudaColor& c) const = 0;

  /**
   * Draw outline with OpenGL (for visualization)
   * \param c desired highlight color
   */
  virtual void drawOutline(const CudaColor& c) const = 0;
